...
> **文档编号**：spec-m3
> **版本**：v1.0
> **状态**：MVP 规格（v1.0.0 已交付；**v1.2.0 / Sprint 6 达成 chunk 级引用溯源**——`:Chunk` 证据 + 引用回查 + 前端原文高亮，受控问题集覆盖率 100%；**≥3 跳遍历待 S10**；**2026-09-24 补做 S6 侧验收对账**——逐条见 `docs/acceptance-traceability-matrix.md` §3.4，缺口见本文件 §6；实现态见 `backend/CODEBUDDY.md` §4）
...
> **关键结论**：**M3 的"可溯源 + 拒答兜底"是产品差异化的最核心体现**。

---

## 6. 已登记的实现缺口（**S6 收尾补做，2026-09-24**）

> 本节按 `docs/dev-doc-status.md` §9.2 第 6 项维护：**实现态与规格的差距必须落在规格侧**。S6 收尾当时未执行 §9.2 第 3 项的逐条对账，本节为**历史补做**（核实日期统一为 2026-09-24）。逐条对账见 `docs/acceptance-traceability-matrix.md` §3.4。

| # | 缺口 | 现状 | 承接 |
|---|---|---|---|
| **S6.4-2** | §3 验收 1 的 **≥ 3 跳遍历** | `graphs.py` **无任何变长路径模式**（`*n..m` 零命中），实际是「实体间 1 跳」与「`Document → Chunk → Entity` 2 跳」两种**定长模式**；§5.4 草案的 `*1..3` 未落地。节点 ≤ 500 + `truncated` 已达成本验收的那部分 | **S10** |
| **S6.4-3** | §4.3 的 **`qa_logs` 表未建** | 至今无该模型（`models.py` 现七张表），导致 §3 验收 3 / 7 的「完整链路打点」**今天仍不成立** | **未排期** |
| **S6.4-4** | §3 验收 3 的 **`qa.refused` 审计** | 拒答出口 `_refuse` 的**契约体**已达成（2 道库外题正确拒答、0 误伤）；审计动作随 M5 未落地 | **S8**（M5） |
| **S6.4-5** | §3 验收 5 的**意图路由**（`route = "m4_affiliation"` + `task_id`） | `route` 在两个出口**恒为 `"m3_graphqa"`**（`agents.py:378 / 506`）；S7 的 M4 疑点链路走**独立端点**，未接回 M3 路由 | **未排期** |
| **S6.4-6** | §3 验收 6 的 UI 文案差异 | `confidence = "low"` 时前端打「**低置信度**」徽标，非 spec 原文「建议人工复核」——**语义一致、字面不同** | 保持（登记不改） |

> **口径说明**：§3 验收 2（引用覆盖率 100%）与验收 4（单 / 跨文档隔离）本次复核**仍成立**（14 问覆盖率 100% / 拒答误伤 0；`single_doc` 走 `_QUERY_EVIDENCE_CHUNKS_BY_DOCUMENT` 双重约束），但其达成前提是**chunk 级而非提及级**——`char_offset` 恒 0 的缺口记在 `specs/m2-extract-kg.md` §6 S6.2-1，两侧共用同一条限制，不重复登记。**这是相对普通 RAG 唯一结构性差异**（多跳）+ 唯一**可验证承诺**（引用覆盖率 100%）。**任何为追求"看起来能答"而放松溯源约束的优化都应被拒绝**——这是 F3 反证条件所防御的。