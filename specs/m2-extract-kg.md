...
| **S6.2-1** | `Citation.char_offset` **恒为 0**（§3 验收 2 要求的提及级定位） | 2026-09-24 复核仍为 0；实体 `char_start` / `char_end` **有值**但未进 citation | **S10** |
| **S6.2-2** | **实体消解未做**：同一实体重复 3 份、整句被抽成实体名（6 → 18 节点） | `entity_merge_candidates` 表**不存在**（§3 验收 3 未做） | **S9 批次 D** |
