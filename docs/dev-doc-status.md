...
**2026-09-26 追加 8（演示前一组动作：¥0 复算 + 按范围提问验证 + 完整彩排）**：① **`--diagnose` 复核（¥0）**：结论与「追加 6」④登记**逐字一致**——`11082/500`、`986 chunks`、`75 可达（7.6%）`、**可达文档 1/6** ⇒ §7.9 的窗口截断**未发生漂移**，登记的数字是**可复现**的（这正是把探针做成复用线上常量脚本的价值：一次性探针会与代码漂移，它不会）。② **按 §7.8 提问范围验证（真实 LLM，¥0.05 量级）**：提问「招商蛇口本期债券的牵头主承销商是哪家机构？」⇒ **`refused=False`、`citations=3`**——即**过渡期方案 c（绕行）实测可行**；反过来说，**问招商公路 / 招商轮船仍会「无法回答」**，演示必须照此范围。③ **首次完整彩排（含真实 LLM + 前端走查）**：`demo_rehearsal.py --with-llm --frontend-base http://127.0.0.1:3000` ⇒ **PASS 9 / FAIL 0 / SKIP 0**（步骤 0 `version=1.4.0`；步骤 1 语料 9/9 completed；步骤 2 `doc=6 / entity=2000 / rel=819` + 抽样 5/5 类型非空；步骤 3 问答如上；步骤 4 溯源试过 3 篇——前 2 篇子图为空、第 3 篇 500 节点，正是该步骤"逐篇试探"设计的价值；步骤 5 疑点 10 条全部带证据链；**步骤 6 同一 trace 15 条、7 类 action 齐全**（黄金路径步骤 6「≥7 条」在**脚本路径**下达成，浏览器路径仍待 S11 的前端 trace 透传）；步骤 7 前端 `/audit`、`/settings` 均 200）。⇒ 「关 Mock 硬门槛」从人工点验升级为**一条命令可复现**。④ **环境收尾**：彩排用的 uvicorn(:8000) 与 `next dev`(:3000) 已停，Neo4j 容器保留（演示要用，一条 `docker start neo4j` 即在）；彩排报告落在 `backend/reports/`（**已被 .gitignore 忽略**，不入库，避免把真机快照当成制品提交）。⑤ 费用：本组动作合计 **≈¥0.05**。

**2026-09-26 追加 7（PR #6 合入 `main`：受控问题集换版 v2 落地）**：① **合并**：PR #6（见「追加 6」）CI **4/4 SUCCESS**（后端 ruff+pytest / 前端 lint+gen:api / 契约漂移 / 流水线汇总）⇒ **squash 合并到 `main`，提交 `6e66101b`**，远端分支 `feat/s8-qset-v2` 随 `--delete-branch` 删除；本地 `main` = `origin/main` = `6e66101b`。② **第二次踩到同一个坑（印证「追加 5」④的教训）**：`gh pr merge` 输出 `✓ Squashed and merged` 之后，紧随的本地同步报 `fatal: unable to access github.com:443` + `not possible to fast-forward`，随后 `git pull --ff-only` **连续失败 6 次**（连接重置 / 超时），**第 7 次成功**。⇒ 判据仍是 `gh pr view 6 --json state` = **`MERGED` / `mergedAt=2026-09-26T10:55:09Z` / `mergeCommit=6e66101b`**，**不是** gh 或 git 的本地同步输出。③ **tag 本次不动（刻意）**：`v1.4.0` 仍指向 `78fa230a`，**不再第 4 次重打**——本次换版是 v1.4.0 **之后**的新增交付，按 §8.1 的新判据（tag 的树须包含 notes 声称的全部交付）它本就不在 v1.4.0 范围内 ⇒ 版本 bump 留到 **S9 / v1.5.0** 一起做，避免为每次补做反复 force push tag。④ **§7.9 裁决已落到排期**：`docs/sprint-calendar.md` 的 **S10 行**已登记「M3 检索覆盖」承接项（含过渡期按方案 c 绕行：S10 之前演示问答只在招商蛇口本期债券范围内提问）。⑤ **仍未做（不粉饰）**：M3 验收判据「引用覆盖率 100% + 拒答 0 误伤」**仍未达成**（换版后实测拒答口径不符 7/14），阻塞项即 §7.9，已裁决归入 S10；**不在收尾期顺手改检索**。

**2026-09-26 追加 6（v1.4.0 收尾续做：tag 第三次重打 + 受控问题集换版 v2 + 新发现检索缺口 §7.9）**：① **tag 第三次重打（我的建议=重打，用户采纳）**：理由与前两次都不同——notes §7.3 已声明「批次 D 三项全部已做」，而补做的 2 笔（`docs/s8-merge-trace` 合入留痕 + 彩排/切片脚本）**落在 tag 之后** ⇒ tag 不含即与 notes **自相矛盾**。已 `git tag -f -a v1.4.0 78fa230a` + force push，远端核 `v1.4.0^{}` = **`78fa230a`**（`is-ancestor` = 0）。**教训入册**：判据不只是「SHA 在不在 main 上」，而是「tag 的树是否包含 notes 声称的全部交付」。② **受控问题集换版 v2（A15 约定的时机已到）**：种子集已固化 ⇒ 按**同一批语料**重出 14 题（12 库内 + 2 库外，规模与 v1 一致保可比），每题登记「出处文档 → 期望答案要点」，冻结 `QSET_VERSION=v2-2026-09-26` / `QSET_KG_VERSION=v-s71a-fe1c4dc3`；新增 `tests/test_eval_qset.py`（12+2 结构 / 不重复 / 版本常量同步改）。③ **实测（真实 LLM，14 题全跑）**：引用覆盖率 **100%**、请求失败 0；拒答口径不符 **7/14 ⇒ 仍 NOT PASS**，**全部是「期望回答、实际拒答」**——招商公路 Q1~Q5 + 招商轮船 Q11~Q12；**招商蛇口 Q6~Q10 全部正常**（引用 2~3 条）。④ **根因（¥0 复算，非猜测）——这是本次最重要的发现**：问答候选集是**结构性**的，`question` **不参与**召回与打分：11082 实体只取扫描序**前 500**（无 `ORDER BY`）⇒ 可达 chunk **75/986（7.6%）**，且**全部来自「招商蛇口_p1-190」1 篇（1/6）** ⇒ 其余 5 篇**必然拒答**。即：**7 条不符与问题集质量无关**。已登记为 notes **§7.9**（含链路、影响、三方案 a/b/c 待裁决，我建议 a 归入 S10）。⑤ **不越界**：这是**检索设计**变更（排序 / 语义召回 / 全实体覆盖 / 按文档取样），且全仓**无任何向量 / 全文索引**（`:Chunk` 连普通索引都没有）⇒ 属新增能力而非 bug 修复，**不在收尾期顺手改**；按 A15 同款纪律也**不**通过删题改题凑 100%。⑥ **新增 ¥0 复算命令**：`uv run python scripts/eval_controlled_qset.py --diagnose`——**复用线上常量与 Cypher**（`_GRAPH_NODE_LIMIT` / `_EVIDENCE_CHUNK_LIMIT` / 两条查询串），改窗口参数立即反映，不像一次性探针会与代码漂移（临时探针 `scripts/_probe_corpus.py` 已删）。⑦ **环境**：彩排时为取证起的 uvicorn(:8000) 与 `next dev`(:3000) 已停（端口监听 0）；Neo4j 容器**保留**（演示要用）。⑧ 费用：本次换版实测约 **¥0.7**（14 题真实 LLM）。⑨ **§7.9 裁决（用户采纳建议 a）**：归入 **S10**，与「证据链多跳 / `:Chunk` 真实引用 / `confidence` 落库」同源做；**过渡期按方案 c 绕行**——S10 之前演示问答只在**招商蛇口本期债券**范围内提问；S10 立项须连带把 `--diagnose` 的「可达 chunk 覆盖率 / 可达文档数」从登记项升级为**验收判据**。已在 `docs/sprint-calendar.md` 的 **S10 行**登记该承接项。

**2026-09-26 追加 5（Sprint 8 合入 `main`：PR #3 squash 合并 + tag 迁回主干提交）**：① **gh 登录**：用户按 A 方案完成 `gh auth login`（浏览器设备码，账号 `csheep86`，token scopes 含 `repo` / `workflow`）——此前 PR 标题 / 描述 / 转正式只能人工在网页点，登录后改由 CLI 执行。② **PR #3 处置**：标题由过时的「Sprint 8.1 批次 A」更正为 **Sprint 8 全量交付**（4 批次 + 2 处收尾期真机修复），描述重写为「范围表 + 门禁证据 + 真机点验结论 + release notes 链接」；draft → **ready for review**；合并前状态 **4/4 SUCCESS**（后端 / 前端 / 契约漂移 / 流水线汇总）、`mergeable=MERGEABLE`。③ **合并**：**squash** → `main` 提交 **`6db0022c`**（PR #3 `MERGED`，远端分支 `feature/sprint-8` 随 `--delete-branch` 删除）。④ **本地 main 的一个坑（已处理，无内容丢失）**：本地 `main` 有 **2 个从未推送**的 docs 提交（`17af6fcf` S7 验收对账 / `2ad1e731` S6 侧 M2·M3 补做对账），直接 `gh pr merge` 因无法 fast-forward 中止（**远端已合并成功**）——说明 gh 的本地同步失败**不代表**合并失败，须以 `gh pr view --json state` 为准。随后 `git rebase origin/main` 触发 3 处冲突（`acceptance-traceability-matrix.md` / `dev-doc-status.md` / `v1.1.0-demo-mvp-plan.md`），逐块核对后确认 **HEAD（已合并 main）侧是更新更全的版本**（含 S6 侧补做 + 2026-09-26 S8 收尾对账），本地侧为旧状态 ⇒ 三处一律取 HEAD；两个提交因内容**已在上游**而变空，被 rebase **自动丢弃**（丢弃前已用 `git cherry -v` 与冲突块逐行核对确认无独有内容）。教训：**本地分支落后 / 分叉时，判据是 `gh pr view --json state`，不是 gh 的本地输出**。⑤ **tag 迁移（第二次重打，理由与上次不同）**：squash 合并生成新提交后，tag `v1.4.0` 指向的 `90c5498a` **不在 `main` 历史内**（`git merge-base --is-ancestor` = 1），但其树与 `main` **完全一致**（`git diff v1.4.0 main` 为空）⇒ `git tag -f -a v1.4.0 6db0022c` + `git push -f origin v1.4.0`（forced update，远端 tag 对象 `c9067988`），迁移后 `is-ancestor` = **0**。结论：**squash 合并会改变提交 SHA，release tag 必须重挂到主干提交**，否则 tag 悬在游离分支上。⑥ **门禁**：合入后 `main` = `origin/main`（无 ahead / behind）；本条追加自身走 PR（`docs/s8-merge-trace`）以保 CI 门禁。⑦ **仍未做（不粉饰）**：批次 D 第 2 项**演示彩排脚本**仍 🟡（与切片脚本化同属降级项，见「追加 4」③）；种子数据集已 ✅（`docs/demo-seed-dataset.md`）。

**2026-09-26 追加 6（批次 D 收口：演示彩排脚本 + 切片脚本化 —— 两项降级项清零）**：① **彩排脚本 `backend/scripts/demo_rehearsal.py`**：闭合 notes §7.2 第 8 项（「关 Mock 硬门槛」此前**只有浏览器点验**，无机械判据）与 §7.3 第 2 项。三处刻意取舍——**默认 ¥0**（步骤 3 提问走真实 LLM，默认 SKIP，`--with-llm` 才跑）；**不改数据**（不上传 / 不建图，属 `demo-seed-dataset.md` §4 重建步骤）；**同一 trace 手工贯穿**（前端 `client.ts` 只读不发 `X-Trace-Id` ⇒ 脚本必须自己透传同一 UUID，否则步骤 6 审计回放不成立，该缺口 → S11）。② **断言不是"200 就过"**：步骤 2 查统计值非 0 + 抽样 5 节点 `entity_type` 非空（正是本版两个真机 bug 的回归守卫）；步骤 4 **逐篇试探**取第一个有子图的文档（真机实况：9 篇 completed 中 **3 篇子图为空**，盲信"第一条"会误判 FAIL——首次跑就红了，据此改的）；步骤 6 要求 6~7 类 `action` **齐全**；步骤 7（`--frontend-base`）核 `/audit` 含「审计」、`/settings` 含「演示环境」。③ **真机证据**（Docker + Neo4j + uvicorn + `USE_MOCK=false` 的 `next dev`）：¥0 模式 **PASS 6 / FAIL 0 / SKIP 2**；完整模式（含真实 LLM）**PASS 9 / FAIL 0 / SKIP 0**、exit 0——步骤 3 `citations=2`、步骤 6 同一 trace **15 条** 7 类 action 齐全、步骤 7 两页 200。④ **切片脚本化 `backend/scripts/slice_demo_corpus.py`**：闭合 `demo-seed-dataset.md` §5 限制 3 前半段（原先靠一次性脚本 + 外部 `qpdf` / `pdftk`）。用 **pypdf**（**dev 依赖组**，非运行时依赖），前置校验原件哈希 / 页数、产出校验 `filename_hash` 与现库一致；**真机 ¥0 实测 PASS 9 / FAIL 0**，且 6 个切片与 §3 表**字节数与 SHA-256 逐字节一致** ⇒ 现库基线**可被脚本复现**（原限制「哈希不可由原件推导」仍成立，「不可复现」不成立）。**仍未脚本化**：上传 → 等解析 → 激活版本（烧费用 + 写演示库，需独立 org 与回滚设计）⇒ 继续登记为未做。⑤ **门禁**：`pytest` **398 → 412 passed**（新增 `tests/test_demo_rehearsal.py` **+14**，用 `httpx.MockTransport` 守「统计值恒 0 必红」「`entity_type` 恒空必红」「默认不调 `/agent/query`」「无子图文档要跳过」「缺 action 必红」+ 切片文件名哈希 6 条参数化）；`ruff check` + `format` 全过；`check_seams` **ERROR 0 / WARN 0 / OK 9**（`--strict` exit 0）；契约零漂移；`gen:api` 无 diff。⑥ **踩坑**：Windows 控制台默认 GBK，脚本里打「¥」与中文会 `UnicodeEncodeError` ⇒ 两脚本入口统一 `sys.stdout.reconfigure(encoding="utf-8")`；`importlib` 动态加载 `scripts/*.py` 时**必须先登记进 `sys.modules` 再 `exec_module`**，否则模块级 `@dataclass` 解析注解会因 `sys.modules[cls.__module__]` 为 `None` 而炸。⑦ **notes 同步**：§6 对账第 8 项 🟡 → ✅；§7.3 标题改「三项全部已做」，新增 §7.3.1 / §7.3.2。
