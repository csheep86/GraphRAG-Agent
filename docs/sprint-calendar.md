...
| S10 | 2026-12-02 → 2026-12-18 | 2.5 周 | `v1.6.0` | 证据链与多跳（`*1..3`）+ `:Chunk` 真实引用 + `confidence` 落库 + docx 解析 + **M3 检索覆盖（v1.4.0 §7.9 裁决 a：候选集由「前 500 实体无排序」改为可召回全量；实测可达 chunk 7.6%、可达文档 1/6）** |
