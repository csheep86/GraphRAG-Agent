...
| **M3-3** | 拒答出口返回契约体，**M5 写 `qa.refused` 审计** | 响应体结构 + 审计落库 | 🟡 **部分达成**：拒答**唯一出口** `_refuse`（`agents.py:480-509`）返回 `answer="无法回答"` / `refused=true` / `refusal_reason` / `confidence="low"`，14 问中的 2 道库外题**全部正确拒答、无误伤**。**缺**：`qa.refused` 审计随 M5 未落地（S8）；`qa_logs` 表（spec §4.3）**未建** | M5 侧 **S8**；`qa_logs` **未排期** |
