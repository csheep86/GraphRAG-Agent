...
| **S7.2-2** `TaskManager.list_in_flight_task_ids()` **只扫 `documents`**（2026-09-24 新增） | 未偿还：`recover_orphan_tasks()` 已扩到扫 `affiliation_tasks`，但该函数仍只查 `documents`（`tasks/manager.py`），对账 `affiliation_tasks` 时**查不到在途任务**。当前无调用方依赖它做 M4 对账，故未顺手改（范围纪律） | 承接 **S8**（与跨阶段投递 S7.1-5 一并收） |
| **S7.1-7** M4「两层并存」未落 release notes（2026-09-24 新增） | 未偿还：`:Entity`（M2 通用实体）与 `:Subject`（M4 主体层）在图里两层并存、未统一，属**已知限制**，必须写进 **v1.3.0 release notes**；但 v1.3.0 notes 属 S7 **四个批次全完**后的收尾动作（不允许在此之前 bump） | 承接 **S7 收尾**（与 `app_version` bump + tag v1.3.0 同一动作）；任务勾选项在 `changes/Sprint7.1/tasks.md` §2.2 保持未勾选以免静默 |
