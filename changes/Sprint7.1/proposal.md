...
| **D6**（新增） | 演示数据集从哪来——算法要「跨公司」命中，须 ≥2 家存在共同法人 / 地址的公司 | ① **2–3 份互为关联的真实公司年报**（母子公司 / 同集团）；② 用 CSV 供应商主数据；③ 单份年报硬凑 | ✅ **拍板 = ①**。②的 CSV 承接在 S9（plan 第 579 行，且 R14 要求 S9 首日先冻结 schema）；③不可行——单份年报只有 1 家公司，跨不了公司。**风险明示**：我尚未验证这几份文档是否真存在可命中的交叉，答案由 7.0 真机验证给出；零交叉就按 plan §6.4 换数据或上报，**不调阈值凑数** |
