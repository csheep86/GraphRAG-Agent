...
> **2026-09-23 后续修正**：用户已人工取得 **招商蛇口 / 招商公路 2024 年公司债募集说明书**，并完成定向切片真机验证，结论见 **§7**——**共享法人交叉成立**，D6② 由「零交叉」修正为「**有交叉（限共享法人一类）**」。
...
## 7. 补充验证：替代素材（募集说明书）定向切片真机（2026-09-23）

§4.4 建议改用「能同时披露多个关联主体的名称 + 法人 + 住所」的材料。用户人工取得 2 份后，本批次补跑定向切片验证（**仍非全量重抽**，单份 ≤8 chunk）。

### 7.1 素材与预处理（零成本）

| 文件 | 页数 | 切片区间 | 字数 | chunk |
|---|---|---|---|---|
| 招商蛇口 2024 公司债（第一期）募集说明书 | 314 | p45–52 | 8368 | 8 |
| 招商公路 2024 科创债（第一期）募集说明书 | 290 | p29–36 | 7876 | 8 |

- PDF 文本提取走 `uv run --with pypdf`（**临时依赖，不写入 `pyproject.toml` / `uv.lock`**）；
- 预处理脚本 `changes/Sprint7.0/slice_prospectus.py`（`scan` 定位关键词页码 → `slice` 生成切片），**不调用 LLM**；
- 抽取仍走 `LangextractClient.from_settings()` + `verify_slice.py`，档位 `llm`。

### 7.2 真机结果（同一入口，无打桩）

**招商蛇口**（243 实体 / 170 关系 / 77.9s / `prompt 20694` + `completion 30012` = **¥0.2815**）

- `PERSON` → **缪建民** ✅（招商局集团法定代表人）
- `VENUE` → **北京市朝阳区建国路118号招商局中心招商局大厦五层A区** ✅（集团注册地址）
- `ORG` → **招商局集团有限公司** ✅

**招商公路**（205 实体 / 94 关系 / 59.6s / `prompt 20938` + `completion 22038` = **¥0.2182**）

- `PERSON` → **缪建民** ✅（同一控股股东招商局集团）
- `ORG` → **招商局集团有限公司** ✅
- `VENUE` → **无**「建国路118号」；抽到的是公路自身地址（天津自贸试验区 / 北京市朝阳区北土城东路 9 号院华丰大厦等）

补充核对（零成本）：公路募集说明书**全文 0 处**命中「建国路」，其控股股东章节只披露集团名称 + 法定代表人，**未披露集团注册地址**。

**第三份候选（招商轮船）零成本核对**——轮船 2024 年未见公司债募集说明书，改用其 **2025 年年报 p90–91（「四、控股股东及实际控制人情况」）**，纯文本提取、未调 LLM：

- 控股股东 **招商局轮船有限公司** → 单位负责人 / 法定代表人 **缪建民**；
- 实际控制人 **招商局集团有限公司** → 法定代表人 **缪建民**，**成立日期 1986-10-14**；
- 与蛇口募集说明书 p46–47 记载的招商局集团成立日期 **1986-10-14 完全一致** → **同名消歧有据**，可确认为同一法人实体（不是同名误报）。
- 轮船年报同样 **0 处**命中「建国路」。

**本轮合计 ¥0.4997**（预算护栏：单次远低于 ¥1）。

### 7.3 D6 结论修正（**批次 A 开工依据以此为准**）

| 结论 | 修正前（§4.4） | 修正后 |
|---|---|---|
| **D6①** 能否抽出法人 / 地址 | 能 | **能**（维持，且募集说明书比年报更有料：多主体并列） |
| **D6②** 文档间是否存在真实交叉 | **零交叉** | **有交叉，但仅限「共享法人」一类**：蛇口 + 公路同以招商局集团为控股股东，法人为同一人（缪建民）。**「共享地址」在这两份之间不成立** |

- **伪交叉已排查**：缪建民是**控股股东的法定代表人**（真实股权关联），不是 §4.4 登记的「东方广场」类中**介机构地址伪交叉**；但批次 A 若产出「共享地址」疑点，仍须先判断是否打到了会计师事务所 / 主承销商地址（本切片里公路就抽到了多家中介地址）。
- **素材形态结论**：**年报不行（零交叉），募集说明书可行**。批次 A 演示数据集应改用 **2–3 份同集团的公司债募集说明书**。

### 7.4 给批次 A 的落地建议

1. **疑点产出已可满足 ≥3 条（无需调阈值）**：三家（蛇口募集说明书 / 公路募集说明书 / 轮船年报 p90–91）**共享同一法定代表人缪建民**，两两组合直接产出 **3 条**「共享法人」疑点：蛇口↔公路、蛇口↔轮船、公路↔轮船。且招商局集团成立日期（1986-10-14）在蛇口与轮船两处一致，**同名消歧有据**。
   - 注：第三份用的是**年报的「控股股东 / 实际控制人」章节**（轮船 2024 年无公司债募集说明书）——这说明**年报并非全无价值**：其「公司简介」部分零交叉，但**控股股东章节含交叉**。批次 A 若坚持全用年报，须把切片/抽取范围限定到该章节，否则仍会落回零交叉。
2. **图建模落点**：`:Subject`(招商局集团) `-[:LEGAL_REP]->` `:LegalPerson`(缪建民) 被两份文档共同指向 → 两跳算法可直接命中，**不依赖地址**。
3. **共享地址疑点**：需确认所选文档是否披露集团注册地址；本次 2 份中仅蛇口披露 → 若只有共享法人类疑点，**不得为凑够 3 条而调阈值**（G5 诚实性）。
4. **成本外推修正**：本轮 8 chunk ≈ ¥0.25（比 §5.2 的 12 chunk ¥0.41 略低，因 completion 更少）。**全量重抽一份募集说明书（290–314 页）仍按 §5.2 口径估算 ¥5–7**，批次 A 预算须按此重新评估。
